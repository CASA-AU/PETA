...
fnRunTAnalysis <- function(dat ## dataset with variables of interest
  ...
  # setup for graph (four per page) in pdf file
	pdf(file=paste0(fOutdir, fanalysisvar, '_trend_analysis_model_plots_'
  ...
  for (svar in varname) { ## for each s(pecfic) var in varname
		...
		if (grepl('^_._', svar)) {
			...
			if (FitFlag %in% c(1, 2, 3, 4)) {
				...
				tmptable <-  data.frame(OccurrenceType              = svar
...
# 				  dat$lexposure <- log(dat[,"exposure"])
        ...
        plot(y = yvar, x = dat$trend
						 , xlab = 'Year_Quarter', ylab = ylab
						 , xaxt = 'n', col = 'black'
						 , pch = 1
				...
				axis(1, at = dat$trend, label = paste(dat$year, dat$quarter, sep='_')
						 , las = 2, cex.axis = 0.7
						 )
				
				## Plot the best fitting model
				lines(bmfitted, type ='l'
							, lty = 1, col = 'blue', lwd=3
							)
				
				flegend <- ifelse(grepl('1', bestmod), 'Trend + LastSpecial'
													, ifelse(grepl('2', bestmod), 'LastSpecial'
																	 , ifelse(grepl('3', bestmod), 'Trend'
																						, 'Constant'
																						)
																	 )
													)
				...
				if (nonlinflag != '') {
					lines(nlfitted, type ='l'
								, lty = 5, col = 'green3', lwd=2
								)
				...
				if (seasonflag == 1) {
					lines(sefitted, type ='l'
								, lty = 4, col = 'magenta', lwd=2
								)
					smodtype <- ifelse(grepl('10', seas$bestseasmod), 'Constant + Seasonal'
														, ifelse(grepl('9', seas$bestseasmod), 'Trend + Seasonal'
																		 , ifelse(grepl('8', seas$bestseasmod), 'LastSpecial + Seasonal'
				...
				## Legends
				legend('topleft', legend=flegend
							 , title = ifelse(overdisflag == 0, 'Poisson models'
	...
	if (!is.na(fOutdir)) {
		write.csv(tagtable
							, file = paste0(fOutdir, fanalysisvar
															, ifelse(fanalysisvar == '', '', '_')
															, 'trend_analysis_table_of_flags_'
