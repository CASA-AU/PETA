...
fnDFBuild <- function(fVarlist = 'Occurence.Type.Description'
  ...
  #### TODO
  ##
  ## This assumes the row.names are formatted year_quarter.
  ##
  tempDate <- as.Date(row.names(datfrm))
  day(tempDate) <- 15  ## NA occurs if you add months() and arrive on a date that doesn't exist.
  tempDate <- tempDate + months(3)
  
  ## Year
  # datfrm$year <- as.numeric(sub('_.*', '', row.names(datfrm)))
  datfrm$year <- year(tempDate)

  #### TODO
  ##
  ## If the 31st of the month is used as a break then a sequence with months
  ## that don't have a 31st day will return NA!!!
  ##
  
  ## Quarter
  # datfrm$quarter <- as.numeric(sub('.*_', '', row.names(datfrm)))
  datfrm$quarter <- month(tempDate)
